...
cGAN.load_weights(os.path.join(checkpoints,"cGAN_model.h5"))
...
for e in range(epochs):
    ...
    for x_batch, y_batch in datagen.flow(rgb, y_train, batch_size=samples):
        ...
        if batches >= 1:
            ...
            break
    if e%5 == 0:
        cGAN.save_weights(os.path.join(checkpoints,str(e)+'.h5')) 
    gen_image_val = gen.predict(gray_val, batch_size=8)
    # if e%1 == 0: 
...
cGAN.save_weights(os.path.join(checkpoints,str(epochs)+".h5"))
